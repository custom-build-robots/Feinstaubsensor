...
# den GPS Koordinaten.
def write_csv(pm_25, pm_10, value_lat, value_lon, value_time, value_temp, value_pressure, value_humidity, value_fname):
	...
	temp = string.replace(value_temp, ".", ",")
	pressure = string.replace(value_pressure, ".", ",")
	humidity = string.replace(value_humidity, ".", ",")
	
	fname = value_fname
	with open(fname,'a') as file:
		line = ""+pm_25+";"+pm_10+";"+lat+";"+lon+";"+time+";"+temp+";"+pressure+";"+humidity
		file.write(line)
		file.write('\n')
		file.close()
	...
	while True:
		...
		while run:
			...
			if -90 <= session.fix.latitude <= 90 and session.fix.latitude != 0:
				...
				pm_old_10 = pm_10
			if single_run:
				write_csv("PM25", "PM10", "GPS FIX Latitude", "GPS FIX Longitude", "Datum;Uhrzeit", "Temperatur (C)", "Hektopascal", "Luftfeuchte", fname_csv)
				single_run = False
			...
			write_csv(str(pm_25), str(pm_10), str(session.fix.latitude), str(session.fix.longitude), datetime.datetime.now().strftime ("%Y%m%d;%H:%M:%S"),str(degrees), str(hectopascals), str(humidity), fname_csv)
...
@app.route('/')
def index():
	return render_template('index_bme280.html')
...
def status():
	...
	ret_data = {"value": status_text, "lat": display_lat, "lon": display_lon, "pm_10": pm_10, "pm_25":pm_25, "error_msg":error_msg, "degrees":round(degrees,2), "pascals":round(hectopascals,2), "humidity":round(humidity,2)}
	return jsonify(ret_data)
